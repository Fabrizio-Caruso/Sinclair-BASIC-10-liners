...
4 if rnd>q then let xx = px -4 + 2*int(rnd*5): let xx = xx - 32*int(xx/32): let xy = py -2 + 2*int(rnd*3):ink 7: print at xy,xx;"o": IF j>=k*100 THEN LET k=k+1:LET f=0
5 ink 2: print at py,px;"*": LET l = j-4*int(j/4):LET s=PEEK(23560):let lr = (s=108)-(s=106): let ud = (s=107)-(s=105): LET y = 0: if s<>13 then LET y = 1: POKE 23560,13
6 if lr or ud then ink 7: print at py,px;"o":let j=j+1:let q=q-0.0005:let px = px+lr: let py = py+ud: let px = px - 32*int(px/32)
7 let gx = (y>0)*(l=3)*int(rnd*32): let gy =(y>0)*(l=3)*int(rnd*21): ink 6: print at gy,gx;"$": IF s=32 THEN IF f<9 THEN print at py-1,px;" ": print at py+1,px;" ": let f = f + 1: print at py,(px<>0)*(px-1);" ": print at py,(px<>31)*px+1;" ":
