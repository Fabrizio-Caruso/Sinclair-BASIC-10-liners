...
4 if rnd>q then let xx = px -4 + 2*int(rnd*5): let xx = xx - 32*int(xx/32): let xy = py -2 + 2*int(rnd*3):print at xy,xx;"o"
5 print at py,px;"*": LET l = j-4*int(j/4):let s=0:let y=0:LET a$=INKEY$:IF a$<>"" THEN LET s=code a$:let e=s-2*int(s/2):IF j>k*100-2 THEN LET k=k+1:LET f=0
6 let c=s-107+e:if sgn(c)=c then print at py,px;"o": let y=1:let j=j+1:let q=q-0.0005:let px = px+(1-e)*c: let py = py+e*c: let px = px - 32*int(px/32)
